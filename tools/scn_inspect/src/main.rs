...
fn analyze(
    ...
    rec_size: usize,
    label: &str,
    known: &dyn Fn(usize) -> Option<&'static str>,
) {
    let ndw = rec_size / 4;
    ...
    println!(
        "{:>5} {:>5} {:>10} {:>8} {:>8}  {:<22} interpretation / distinct",
        "off", "hex", "nonzero%", "floats", "smallint", "field"
    );
    for d in 0..ndw {
        let s = &stats[d];
        if s.total == 0 {
        ...
        if s.nonzero == 0 {
            ...
            let looks_enum =
                s.distinct_u32.len() <= 12 && s.int_small as f32 / s.nonzero.max(1) as f32 > 0.7;
            if looks_float {
        ...
        println!(
            "{off:>5} {off:>#5x} {nz_pct:>9.1}% {:>8} {:>8}  {kname:<22} {interp}",
            s.float_like, s.int_small
        );
    }
}
...
fn main() -> anyhow::Result<()> {
    ...
    for ent in std::fs::read_dir(&scene_dir)? {
        ...
        if let Ok(want) = std::env::var("TREE_LIST") {
            if path
                .file_name()
                .and_then(|f| f.to_str())
                .map(|f| f.to_lowercase().contains(&want.to_lowercase()))
                .unwrap_or(false)
            {
                fn walk(vfs: &MiniFs, dir: &Path, depth: usize) {
                    if let Ok(entries) = vfs.entries(dir) {
                        ...
                        for e in v {
                            ...
                            let sz = vfs.open(&p).ok().and_then(|mut f| {
                                let mut b = vec![];
                                std::io::Read::read_to_end(&mut f, &mut b)
                                    .ok()
                                    .map(|_| b.len())
                            });
                            println!(
                                ...
                                sz.map(|s| format!("({s})")).unwrap_or_default()
                            );
                            if matches!(e.kind, mini_fs::EntryKind::Dir) {
                                walk(vfs, &p, depth + 1);
                            }
                        }
                    }
        ...
        if std::env::var("DKL_DECODE").is_ok() {
            ...
            for (n, b) in files.into_iter().take(2) {
                ...
                for i in 0..count.min(4) {
                    let o = 0x0c + i * 64;
                    let name: String = b[o..o + 32]
                        .iter()
                        .take_while(|&&c| c != 0 && c != 0xcc)
                        .map(|&c| c as char)
                        .collect();
                    println!("  atom[{i}] = {name:?}");
                }
                ...
                for row in 0..16 {
                    ...
                    let hex: Vec<String> =
                        b[o..o + 16].iter().map(|x| format!("{x:02x}")).collect();
        ...
        if std::env::var("SCN_HEADER").is_ok() {
            ...
            for (n, b) in scns.into_iter().take(4) {
                ...
                println!(
                    "   (global region 0x7e..{:#x} = {} bytes)",
                    role_off,
                    role_off as usize - 0x7e
                );
            }
            return Ok(());
        }
        if let Ok(cpkpath) = std::env::var("MV3_NORM_CPK") {
            ...
            let mut vfs2 = MiniFs::new(false);
            vfs2 = vfs2.mount("/", fs2);
            let mut mv3s = vec![];
            collect_ext(&vfs2, Path::new("/"), ".mv3", &mut mv3s);
            let want = std::env::var("MV3_NORM").unwrap_or_default();
            for (n, b) in mv3s {
                if !want.is_empty() && !n.to_lowercase().contains(&want.to_lowercase()) {
                    continue;
                }
                let mv3 = match fileformats::mv3::read_mv3(&mut std::io::Cursor::new(&b)) {
                    Ok(x) => x,
                    Err(_) => continue,
                };
                ...
                let verts: Vec<(f32, f32, f32)> = frame
                    .vertices
                    .iter()
                ...
                for t in &mesh.triangles {
                    ...
                    let e1 = (
                        verts[bb].0 - verts[a].0,
                        verts[bb].1 - verts[a].1,
                        verts[bb].2 - verts[a].2,
                    );
                    let e2 = (
                        verts[c].0 - verts[a].0,
                        verts[c].1 - verts[a].1,
                ...
                // Mesh centroid
                let mut c = (0f32, 0f32, 0f32);
                for v in &verts {
                    c.0 += v.0;
                    c.1 += v.1;
                    c.2 += v.2;
                }
                let nv = verts.len() as f32;
                c = (c.0 / nv, c.1 / nv, c.2 / nv);
                ...
                for t in &mesh.triangles {
                    ...
                    let e2 = (
                        verts[cc].0 - verts[a].0,
                        verts[cc].1 - verts[a].1,
                        verts[cc].2 - verts[a].2,
                    );
                    let cr = (
                        e1.1 * e2.2 - e1.2 * e2.1,
                        e1.2 * e2.0 - e1.0 * e2.2,
                    ...
                    let fc = (
                        (verts[a].0 + verts[bb].0 + verts[cc].0) / 3.0 - c.0,
                        (verts[a].1 + verts[bb].1 + verts[cc].1) / 3.0 - c.1,
                        (verts[a].2 + verts[bb].2 + verts[cc].2) / 3.0 - c.2,
                    );
                ...
                println!(
                    "  manifold winding: consistent_edges={} inconsistent={} boundary={}",
                    consistent, inconsistent, boundary
                );
                        ...
                        for signs in 0..8u8 {
                            ...
                            if avg > best.0 {
                                best = (
                                    avg,
                                    format!(
                                        "incl_from_theta={} perm={:?} signs={:?}",
                                        incl_from_theta, perm, sg
                                    ),
                                    ag,
                                    fl,
                                );
        ...
        if std::env::var("LM_STATS").is_ok() {
            let mut files = vec![];
            collect_ext(&vfs, Path::new("/"), ".dds", &mut files);
            let mut n_lm = 0;
            for (n, b) in files {
                let fname = std::path::Path::new(&n)
                    .file_name()
                    .unwrap()
                    .to_string_lossy()
                    .to_lowercase();
                if !fname.starts_with("^l_") {
                    continue;
                }
                if let Ok(img) = image::load_from_memory(&b) {
                    let rgb = img.to_rgb8();
                    let (mut r, mut g, mut bl) = (0u64, 0u64, 0u64);
                    let px = rgb.pixels().len() as u64;
                    for p in rgb.pixels() {
                        r += p[0] as u64;
                        g += p[1] as u64;
                        bl += p[2] as u64;
        ...
        if let Ok(want) = std::env::var("LGT_ONE") {
            let mut files = vec![];
            collect_ext(&vfs, Path::new("/"), ".lgt", &mut files);
            for (n, b) in files {
                ...
                for i in 0..count {
                    ...
                    println!(
                        "  L{i}: pos=({:.0},{:.0},{:.0}) color=({:.3},{:.3},{:.3}) R-B={:+.3} {}",
                        pos.0,
                        pos.1,
                        pos.2,
                        col.0,
        ...
        if std::env::var("LGT_WARMTH").is_ok() {
            ...
            for (n, b) in files {
                ...
                for i in 0..count {
                    ...
                    sum.1 += c.1;
                    sum.2 += c.2;
                    if c.0 + c.1 + c.2 > maxc.0 + maxc.1 + maxc.2 {
                        maxc = c;
                    }
                }
                println!(
                    "{n} cnt={} brightest=({:.3},{:.3},{:.3}) avg=({:.3},{:.3},{:.3})",
                    count,
                    maxc.0,
                    maxc.1,
                    maxc.2,
        ...
        if std::env::var("LGT_DECODE").is_ok() {
            ...
            for (n, b) in lgts {
                ...
                for i in 0..count {
                    ...
                    let r2 = f(o + 0x78);
                    let cone = f(o + 0x7c);
                    let dir = (f(o + 0x88), f(o + 0x8c), f(o + 0x90));
        ...
        if std::env::var("DKL_FINDVERT").is_ok() {
            ...
            for (pn, pb) in pols {
                if !pn.to_lowercase().contains(&want.to_lowercase()) {
                    continue;
                }
                let mut cur = std::io::Cursor::new(&pb);
                let pf = match fileformats::pol::read_pol(&mut cur) {
                    Ok(x) => x,
                    Err(_) => continue,
                };
                ...
                let read = |q: std::path::PathBuf| {
                    vfs.open(&q).ok().and_then(|mut f| {
                        let mut b = vec![];
                        std::io::Read::read_to_end(&mut f, &mut b).ok().map(|_| b)
                    })
                };
                let dkl = match read(dir.join(format!("{stem}.dkl")))
                    .or_else(|| read(dir.join(format!("{stem}.DKL"))))
                {
                    Some(x) => x,
                    None => continue,
                };
                ...
                let find = |needle: &[u8], from: usize| -> Option<usize> {
                    let mut i = from;
                    while i + needle.len() <= dkl.len() {
                        if &dkl[i..i + needle.len()] == needle {
                            return Some(i);
                        }
                        i += 1;
                    }
                ...
                for (vi, v) in m.vertices.iter().take(8).enumerate() {
                    let mut nb = vec![];
                    nb.extend_from_slice(&v.position.x.to_le_bytes());
                    nb.extend_from_slice(&v.position.y.to_le_bytes());
                    nb.extend_from_slice(&v.position.z.to_le_bytes());
                    if let Some(off) = find(&nb, 0) {
                        println!(
                            "  POLv{vi} ({:.2},{:.2},{:.2}) -> dkl @ {:#x}",
                            v.position.x, v.position.y, v.position.z, off
                        );
                        hits.push(off);
                    } else {
                        println!(
                            "  POLv{vi} ({:.2},{:.2},{:.2}) -> NOT FOUND (xyz contiguous)",
                            v.position.x, v.position.y, v.position.z
                        );
                    }
                }
                if hits.len() >= 2 {
                    let d: Vec<i64> = hits.windows(2).map(|w| w[1] as i64 - w[0] as i64).collect();
                    println!("  hit deltas: {:?}", d);
                }
                ...
                if let Some(nrm) = &m.vertices[0].normal {
                    let mut nb = vec![];
                    nb.extend_from_slice(&nrm.x.to_le_bytes());
                    nb.extend_from_slice(&nrm.y.to_le_bytes());
                    nb.extend_from_slice(&nrm.z.to_le_bytes());
                    println!(
                        "  POLv0 normal ({:.3},{:.3},{:.3}) contiguous -> {:?}",
                        nrm.x,
                        nrm.y,
                        nrm.z,
                        find(&nb, 0).map(|o| format!("{:#x}", o))
                    ...
                    println!(
                        "  POLv0 normal.x alone -> {:?}",
                        find(&nrm.x.to_le_bytes(), 0).map(|o| format!("{:#x}", o))
                    );
                }
        ...
        if std::env::var("DKL_STRIDE").is_ok() {
            ...
            for (n, b) in files {
                ...
                let u =
                    |o: usize| u32::from_le_bytes([b[o], b[o + 1], b[o + 2], b[o + 3]]) as usize;
                let cnt = u(0x8);
                let palstart = 0xc + cnt * 64;
        ...
        if std::env::var("DKL_MAP").is_ok() {
            ...
            for (n, b) in files {
                ...
                println!(
                    "{n} len={} meshcount={} data_off={:#x} ArtXufei_strings={}",
                    n.len(),
                    cnt,
                    data,
                    occ
                ...
                let pcnt = u(data);
                println!("  palette_count@data={}", pcnt);
                ...
                if offs.len() >= 3 {
                    let d01 = offs[1] - offs[0];
                    let d12 = offs[2] - offs[1];
                    println!(
                        "  first record strides: {} {} (first3 offs {:#x} {:#x} {:#x})",
                        d01, d12, offs[0], offs[1], offs[2]
                    );
                }
                // where does the last Art-xufei end? what's after?
                if let Some(&last) = offs.last() {
                    println!(
                        "  last ArtXufei @ {:#x}; tail bytes = {} ({}% of data)",
                        last,
                        b.len() - last,
                        100 * (b.len() - last) / (b.len() - data)
                    );
                }
            }
            return Ok(());
        ...
        if std::env::var("TEX_CMP").is_ok() {
            ...
            for (pn, pb) in pols {
                ...
                for (mi, m) in pf.meshes.iter().take(4).enumerate() {
                    for mat in &m.material_info {
                        let names: Vec<String> = mat
                            ...
                            .collect();
                        println!(
                            "  POL mesh{mi} mat: use_alpha={} surf16={:?} tex={:?}",
                            mat.use_alpha, mat.unknown_68, names
                        );
                    }
                }
                ...
                let read = |p: std::path::PathBuf| {
                    vfs.open(&p).ok().and_then(|mut f| {
                        let mut b = vec![];
                        std::io::Read::read_to_end(&mut f, &mut b).ok().map(|_| b)
                    })
                };
                if let Some(d) = read(dir.join(format!("{stem}.dkl")))
                    .or_else(|| read(dir.join(format!("{stem}.DKL"))))
                {
        ...
        if std::env::var("POL_VS_DK").is_ok() {
            ...
            for (pn, pb) in pols {
                ...
                print!(
                    "{pn}: meshes={} verts={} mats={} geomnodes={}",
                    pf.meshes.len(),
                    total_verts,
                    total_mats,
                    pf.geom_node_descs.len()
                );
                if let Some(d) = &dkl {
                    print!(
                        " | dkl: h0={:#x} h1={:#x} cnt={} len={}",
                        u32at(d, 0),
                        u32at(d, 4),
                        u32at(d, 8),
                        d.len()
                ...
                if let Some(d) = &dkm {
                    print!(
                        " | dkm: magic={:?} v={:#x} a={} b={} sz={:#x} cnt@0x4c={} len={}",
                        std::str::from_utf8(&d[0..4]).unwrap_or("?"),
                        u32at(d, 4),
                        u32at(d, 8),
                        u32at(d, 0xc),
                        u32at(d, 0x10),
            ...
            for (n, b) in pols {
                ...
                if let Ok(pf) = fileformats::pol::read_pol(&mut cur) {
                    for (gi, g) in pf.meshes.iter().enumerate() {
                        ...
                        if vt.has(P::UNKNOWN4) {
                            bits.push("U4");
                        }
                        if vt.has(P::UNKNOWN8) {
                            bits.push("U8");
                        }
                        if vt.has(P::TEXCOORD) {
                            bits.push("T1");
                        }
                        if vt.has(P::TEXCOORD2) {
                            bits.push("T2");
                        }
                        if vt.has(P::UNKNOWN40) {
                            bits.push("U40");
                        }
                        if vt.has(P::UNKNOWN80) {
                            bits.push("U80");
                        }
                        if vt.has(P::UNKNOWN100) {
                            bits.push("U100");
                        }
        ...
        if let Ok(ext) = std::env::var("DUMP_EXT") {
            ...
            let max_rows: usize = std::env::var("ROWS")
                .ok()
    ...
    println!(
        "Parsed {file_count} .scn files | {} roles | {} nodes",
        all_roles.len(),
        all_nodes.len()
    );
    println!(
        "night scenes: {night_count}/{file_count} | skybox ids seen: {:?}",
        skybox_ids
    );
