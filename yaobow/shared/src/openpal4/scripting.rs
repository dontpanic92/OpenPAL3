...
use super::vm_context::Pal4VmContext;
...
fn imm_begin(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    vm.imm = true;
    Pal4FunctionState::Completed
}

fn imm_end(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    vm.imm = false;
    Pal4FunctionState::Completed
}

/// Resolve a script-supplied player id to a persistent-state party
/// slot. `-1` means "current leader", matching `Pal4VmContext`'s own
/// `map_player` convention.
fn map_player_slot(vm: &ScriptVm<Pal4VmContext>, player_id: i32) -> usize {
    if player_id < 0 {
        vm.vm_context.leader()
    } else {
        player_id as usize
    }
}

fn new_game(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    // Start from a clean slate so a fresh playthrough doesn't inherit
    // money / inventory / plot flags from a previous session.
    let app_name = vm.vm_context.persistent_state().app_name().to_string();
    vm.vm_context.set_persistent_state(
        crate::openpal4::states::persistent_state::Pal4PersistentState::new(app_name),
    );
    Pal4FunctionState::Completed
}

fn camera_ctrl_ypr(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, _yaw: f32, _pitch: f32, _roll: f32, _is_instant: i32);
    Pal4FunctionState::Completed
}

fn camera_ctrl_dist(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, _dist: f32, _is_instant: i32);
    Pal4FunctionState::Completed
}

fn camera_ctrl_yprd(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, _yaw: f32, _pitch: f32, _roll: f32, _dist: f32, _is_instant: i32);
    Pal4FunctionState::Completed
}

fn camera_get_dist(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    let cam = vm.vm_context.camera_position();
    let target = vm.vm_context.leader_pos();
    let dist = Vec3::sub(&cam, &target).norm();
    vm.set_ret_value(dist);
    Pal4FunctionState::Completed
}

fn camera_get_yaw(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    let e = vm.vm_context.camera_euler_deg();
    vm.set_ret_value(e.y);
    Pal4FunctionState::Completed
}

fn camera_get_pitch(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    let e = vm.vm_context.camera_euler_deg();
    vm.set_ret_value(e.x);
    Pal4FunctionState::Completed
}

fn camera_get_roll(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    let e = vm.vm_context.camera_euler_deg();
    vm.set_ret_value(e.z);
    Pal4FunctionState::Completed
}

fn arena_load(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(
        vm,
        scn_str: i32,
        block_str: i32,
...
fn arena_ready_restore(_: &str, _vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    Pal4FunctionState::Completed
}

fn arena_hint(_: &str, _vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    Pal4FunctionState::Completed
}

fn arena_come_from_here(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, _arena_name: i32, _come_from_here_name: i32);
    vm.set_ret_value(1);
    Pal4FunctionState::Completed
}

fn player_set_leader(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, leader_id: i32);

    vm.vm_context.set_leader(leader_id);

    Pal4FunctionState::Completed
}

fn player_set_visible(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
...
fn player_attach_collision(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, _player_id: i32);
    Pal4FunctionState::Completed
}

fn player_detach_collision(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, _player_id: i32);
    Pal4FunctionState::Completed
}

fn player_lock(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    vm.vm_context.lock_player(true);

    Pal4FunctionState::Completed
}

fn player_unlock(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    vm.vm_context.lock_player(false);

    Pal4FunctionState::Completed
}

fn set_npc_visible(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, npc_name: i32, is_visible: i32);

    let npc_name = get_str(vm, npc_name as usize).unwrap();
    vm.vm_context.enable_npc(&npc_name, is_visible != 0);

    Pal4FunctionState::Completed
}

fn npc_create(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, _npc_name: i32, _behaviour_name: i32, _x_pos:f32,_y_pos:f32,_z_pos:f32);
    Pal4FunctionState::Completed
}

fn npc_delete(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm,_npc_name:i32);
    Pal4FunctionState::Completed
}

fn system_exchange(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm,_exchange_type:i32);
    Pal4FunctionState::Completed
}

fn monster_stop_pursuit(_: &str, _vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    Pal4FunctionState::Completed
}

fn camera_set_mode(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm,_camera_mode:i32,_is_instant:i32);
    Pal4FunctionState::Completed
}

fn get_goods_open_condition(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm,_goods_id:i32);
    vm.set_ret_value(1);
    Pal4FunctionState::Completed
}

fn npc_pause_beh(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm,_npc_name:i32);
    Pal4FunctionState::Completed
}

fn npc_resume_beh(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm,_npc_name:i32);
    Pal4FunctionState::Completed
}

fn open_weather(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm,_weather_type:i32);
    Pal4FunctionState::Completed
}

fn close_weather(_: &str, _vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    Pal4FunctionState::Completed
}

fn set_minimap_expmode(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm,_exp_mode:i32);
    Pal4FunctionState::Completed
}

fn get_randnum(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, min: i32, max: i32);
    use rand::Rng;
...
fn set_temp_game_state(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm,_state:i32);
    Pal4FunctionState::Completed
}

fn flush_tail_y_angle(_: &str, _vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    Pal4FunctionState::Completed
}

fn add_combat_monster(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm,_monster_id:i32,_monster_type:i32);
    Pal4FunctionState::Completed
}

fn config_combat_param(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
...
fn config_combat_bgm(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm,_bgm_name:i32);
    Pal4FunctionState::Completed
}

fn config_combat_camera(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm,_camera_name:i32);
    Pal4FunctionState::Completed
}

fn start_combat(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm,_combat_id:i32);
    Pal4FunctionState::Completed
}

fn set_object_visible(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, object_name: i32, is_visible: i32);

    let object_name = get_str(vm, object_name as usize).unwrap_or_default();
    vm.vm_context.enable_object(&object_name, is_visible != 0);
...
fn player_out_team(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, player_id: i32, _is_in_team: i32);
    let slot = map_player_slot(vm, player_id);
    vm.vm_context
        .persistent_state_mut()
        .set_in_team(slot, false);
    vm.vm_context.enable_player(slot, false);
    Pal4FunctionState::Completed
}

fn gom_touch(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, _file_str: i32);
    Pal4FunctionState::Completed
}

fn camera_set_collide(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
...
fn camera_auto_seek(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, _auto_seek: i32);
    Pal4FunctionState::Completed
}

fn player_set_attr(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, _attr1: i32, _attr2: i32, _attr3: i32);
    Pal4FunctionState::Completed
}

fn player_get_leader(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    vm.set_ret_value(vm.vm_context.leader() as i32);
    Pal4FunctionState::Completed
}

fn set_player_level(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
...
fn add_player_equip(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
...
fn open_movie_flag(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, _flag_id: i32);
    Pal4FunctionState::Completed
}

fn add_quest_complete_percentage(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
...
fn add_equipment(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
...
fn player_current_set_visible(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, is_visible: i32);
    let leader = vm.vm_context.leader();
    vm.vm_context.enable_player(leader, is_visible != 0);
    Pal4FunctionState::Completed
}

fn set_full_hp(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    let leader = vm.vm_context.leader();
    vm.vm_context.persistent_state_mut().set_full_hp(leader);
    Pal4FunctionState::Completed
}

fn set_full_mp(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    let leader = vm.vm_context.leader();
    vm.vm_context.persistent_state_mut().set_full_mp(leader);
    Pal4FunctionState::Completed
}

fn get_player_level(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, player_id: i32);
    let slot = map_player_slot(vm, player_id);
    let level = vm.vm_context.persistent_state().player_level(slot);
    vm.set_ret_value(level);
    Pal4FunctionState::Completed
}

fn goto_logo(_: &str, _vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    Pal4FunctionState::Completed
}

fn player_unhold_act(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, player_id: i32);
...
fn camera_set_dist_opt_enable(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm,_enable_dist_opt :i32);
    Pal4FunctionState::Completed
}

fn monster_set_hide(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm,_file_str:i32,_hide_monster :i32);
    Pal4FunctionState::Completed
}

fn game_object_set_research(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm,_file_str:i32,_research :i32);
    Pal4FunctionState::Completed
}

fn set_portrait(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, file_str: i32, left: i32);
    let file_name = get_str(vm, file_str as usize).unwrap();
    vm.vm_context.set_portrait(&file_name, left != 0);
...
fn script_music_mute(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, _mute: i32);
    Pal4FunctionState::Completed
}

fn script_music_play(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
...
fn world_map_set_state(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, _file_str: i32, _state_id: i32);
    Pal4FunctionState::Completed
}

fn get_puzzle_game_result(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    vm.set_ret_value(1);
    Pal4FunctionState::Completed
}

fn always_jump(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, _jump: i32);
    Pal4FunctionState::Completed
}

fn sound_2d_play(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, name_str: i32, _times: i32);

    let name = get_str(vm, name_str as usize).unwrap();
    match vm.vm_context.play_sound(&name) {
...
fn sound_2d_stop(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    vm.vm_context.stop_all_sounds();
    Pal4FunctionState::Completed
}

fn sound_2d_stop_id(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, sound_id: i32);
    vm.vm_context.stop_sound(sound_id);

    Pal4FunctionState::Completed
}

fn cg_eff_play(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, _eff_id: i32);
    Pal4FunctionState::Completed
}

fn cg_eff_stop(_: &str, _vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    Pal4FunctionState::Completed
}

fn effect_play(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, _file_str:i32,_effect_id:i32,_x:f32,_y:f32,_z:f32);
    Pal4FunctionState::Completed
}

fn effect_play_with_player(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
...
fn effect_play_with_npc(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm,_file_str:i32,_effect_id:i32,_npc_file_str:i32);
    Pal4FunctionState::Completed
}

fn effect_play_with_obj(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm,_file_str:i32,_effect_id:i32,_obj_file_str:i32);
    Pal4FunctionState::Completed
}

fn effect_stop_with_obj(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm,_obj_file_str:i32);
    Pal4FunctionState::Completed
}

fn show_hint(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm,_hint_file_str:i32,_x:f32,_y:f32);
    Pal4FunctionState::Completed
}

fn player_add_skill(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
...
fn monster_set_visible(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm,_monster_file_str:i32,_visible_monster:i32);
    Pal4FunctionState::Completed
}

fn player_random_position(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm,_player_id:i32,_x:f32,_y:f32);
    Pal4FunctionState::Completed
}

fn player_current_random_position(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm,_x:f32,_y:f32);
    Pal4FunctionState::Completed
}

fn event_volume_visible(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm,_event_file_str:i32,_visible_event:i32);
    Pal4FunctionState::Completed
}

fn all_player_garb2(_: &str, _vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    Pal4FunctionState::Completed
}

fn player_garb2(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm,_player_id:i32);
    Pal4FunctionState::Completed
}

fn all_player_garb1(_: &str, _vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    Pal4FunctionState::Completed
}

fn player_garb1(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm,_player_id:i32);
    Pal4FunctionState::Completed
}
...
fn grant_system_ui(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm,_ui_id:i32,_grant_ui:i32);
    Pal4FunctionState::Completed
}

fn open_system_ui(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm,_ui_id:i32);
    Pal4FunctionState::Completed
}

fn grant_smith_system(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm,_smith1:i32,_smith2:i32,_smith3:i32,_smith4:i32);
    Pal4FunctionState::Completed
}

fn grant_magic_system(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm,_magic1:i32,_magic2:i32);
    Pal4FunctionState::Completed
}

fn check_magic_mastered(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
...
fn select_dialog_add_item(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, item_file_str: i32);
    if let Some(item) = get_str(vm, item_file_str as usize) {
        vm.vm_context.session().push_dialog_choice(item);
    }
    Pal4FunctionState::Completed
}
...
fn gob_attach_to_player(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm,_gob_file_str:i32,_attach_file_str:i32,_player_id:i32,_attach_gob:i32);
    Pal4FunctionState::Completed
}

fn gob_attach_to_current_player(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm,_gob_file_str:i32,_attach_file_str:i32,_attach_gob:i32);
    Pal4FunctionState::Completed
}

fn gob_detach_from_player(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm,_player_id:i32);
    Pal4FunctionState::Completed
}
...
fn effect_attach_to_player(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
...
fn effect_detach_from_player(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm,_player_id:i32);
    Pal4FunctionState::Completed
}
...
fn effect_attach_to_npc(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, _npc_file_str: i32, _effect_file_str: i32, _attach_effect: i32);
    Pal4FunctionState::Completed
}

fn effect_detach_from_npc(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, _npc_file_str: i32);
    Pal4FunctionState::Completed
}

fn gob_attach_to_npc(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, _gob_file_str: i32, _attach_file_str: i32, _npc_file_str: i32, _attach_gob: i32);
    Pal4FunctionState::Completed
}

fn gob_detach_from_npc(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, _npc_file_str: i32);
    Pal4FunctionState::Completed
}

fn gob_set_position(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
...
fn script_clear_ctx_but_current(_: &str, _vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    Pal4FunctionState::Completed
}
...
fn hide_ga_skill_object(_: &str, _vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    Pal4FunctionState::Completed
}

fn show_signpost(_: &str, _vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    Pal4FunctionState::Completed
}

fn player_set_emotion(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, _player_id: i32, _emotion_file_str: i32);
    Pal4FunctionState::Completed
}

fn player_reset_emotion(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, _player_id: i32);
    Pal4FunctionState::Completed
}

fn player_current_set_emotion(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, _emotion_file_str: i32);
    Pal4FunctionState::Completed
}

fn player_current_reset_emotion(_: &str, _vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    Pal4FunctionState::Completed
}

fn lingsha_legs_injured(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, _injured_file_str: i32);
    Pal4FunctionState::Completed
}

fn lingsha_legs_healing(_: &str, _vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    Pal4FunctionState::Completed
}

fn npc_set_emotion(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, _npc_file_str:i32,_emotion_file_str:i32);
    Pal4FunctionState::Completed
}

fn npc_reset_emotion(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm,_npc_file_str:i32);
    Pal4FunctionState::Completed
}
...
fn player_current_get_pos_x(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    let p = vm.vm_context.get_player_pos(-1);
    vm.set_ret_value(p.x);
    Pal4FunctionState::Completed
}

fn player_current_get_pos_y(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    let p = vm.vm_context.get_player_pos(-1);
    vm.set_ret_value(p.y);
    Pal4FunctionState::Completed
}

fn player_current_get_pos_z(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    let p = vm.vm_context.get_player_pos(-1);
    vm.set_ret_value(p.z);
    Pal4FunctionState::Completed
}

fn arena_skill_enable(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, _enable: i32);
    Pal4FunctionState::Completed
}

fn show_inn_dialog(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, _show: i32);
    Pal4FunctionState::Completed
}

fn get_inn_dialog_result(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    vm.set_ret_value(1);
    Pal4FunctionState::Completed
}

fn player_take_a_rest(_: &str, _vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    Pal4FunctionState::Completed
}

fn is_night_time(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    vm.set_ret_value(1);
    Pal4FunctionState::Completed
}
...
fn player_set_pos_rot_npc(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, npc_str: i32, x: f32, y: f32, z: f32, rot: f32);
    let name = get_str(vm, npc_str as usize).unwrap_or_default();
    vm.vm_context.npc_set_pos(&name, &Vec3::new(x, y, z));
    vm.vm_context.npc_set_ang(&name, rot);
    Pal4FunctionState::Completed
}

fn start_ui_timer(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, _timer_id: i32, _timer_file_str: i32);
    Pal4FunctionState::Completed
}

fn player_forbiden_skill(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, _player_id: i32, _skill_id: i32, _forbiden_skill: i32);
    Pal4FunctionState::Completed
}

fn get_money(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    let money = vm.vm_context.persistent_state().money();
...
fn show_quest_dialog(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm,_quest_file_str:i32);
    Pal4FunctionState::Completed
}

fn get_quest_dialog_result(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    vm.set_ret_value(1);
    Pal4FunctionState::Completed
}

fn reset_player_to_jump_start(_: &str, _vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    Pal4FunctionState::Completed
}

fn gob_reset(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
...
fn check_equip_in_inventory(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, equip_id: i32);
    let has = vm.vm_context.persistent_state().has_equipment(equip_id);
    vm.set_ret_value(if has { 1 } else { 0 });
    Pal4FunctionState::Completed
}

fn remove_equipment(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
...
fn add_prescription(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm,_prescription_id:i32,_add_prescription :i32);
    Pal4FunctionState::Completed
}

fn enable_shadow(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm,_enable_shadow :i32);
    Pal4FunctionState::Completed
}

fn add_round_times(_: &str, _vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    Pal4FunctionState::Completed
}
...
fn config_combat_ground_camera(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm,_camera_file_str:i32);
    Pal4FunctionState::Completed
}

fn add_player_favor(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm,_player_id:i32,_favor_id:i32,_favor_value:i32);
    Pal4FunctionState::Completed
}

fn get_pal_test_result(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm,_test_id:i32);
    vm.set_ret_value(1);
    Pal4FunctionState::Completed
}

fn set_minimap_level(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm,_level_id:i32);
    Pal4FunctionState::Completed
}

fn pet_show(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm,_show_pet:i32);
    Pal4FunctionState::Completed
}

fn npc_attach_effect(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm,_npc_file_str:i32,_effect_file_str:i32,_effect_id:i32);
    Pal4FunctionState::Completed
}

fn npc_detach_effect(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm,_npc_file_str:i32);
    Pal4FunctionState::Completed
}

fn mst_attach_effect(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm,_mst_file_str:i32,_effect_file_str:i32,_effect_id:i32);
    Pal4FunctionState::Completed
}

fn mst_detach_effect(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm,_mst_file_str:i32);
    Pal4FunctionState::Completed
}

fn player_hook_effect(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm,_player_id:i32,_effect_file_str:i32,_effect_id:i32);
    Pal4FunctionState::Completed
}

fn player_detach_effect(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm,_player_id:i32);
    Pal4FunctionState::Completed
}
...
fn config_combat_vip_monster(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, _monster_id: i32);
    Pal4FunctionState::Completed
}

fn enable_sts(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, _sts_id: i32);
    Pal4FunctionState::Completed
}

fn clear_ui_timer(_: &str, _vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    Pal4FunctionState::Completed
}

fn pause_ui_timer(_: &str, _vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    Pal4FunctionState::Completed
}

fn resume_ui_timer(_: &str, _vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    Pal4FunctionState::Completed
}

fn set_full_rage(_: &str, _vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    Pal4FunctionState::Completed
}

fn ui_timer_get_save_data(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    vm.set_ret_value(1);
    Pal4FunctionState::Completed
}
...
fn wait(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
...
fn talk(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    ...
    if let Some(voice) = vm.stack_peek::<i32>() {
        ...
        if let Some(voice_name) = voice_name {
            if voice_name.len() == 5 && voice_name.starts_with("4") {
                vm.stack_pop::<i32>();
                if let Err(e) = vm.vm_context.play_voice(&voice_name) {
    ...
    vm.vm_context.dialog_box.set_text(&text);
    let presenter = DialogBoxPresenter::new();

    Pal4FunctionState::Yield(Box::new(move |vm, delta_sec| {
        let ui = &vm.vm_context().ui;
        presenter.update(&vm.vm_context.dialog_box, delta_sec);

        let fast = vm.vm_context().fast_forward();
        let input = vm.vm_context().input.clone();
        ...
        if completed {
            ...
            vm.vm_context.dialog_box.close();
            ContinuationState::Completed
        } else {
            ContinuationState::Loop
        }
...
fn rand_talk_push(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, _talk_file_str: i32);
    Pal4FunctionState::Completed
}

fn rand_talk(_: &str, _vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    Pal4FunctionState::Completed
}

fn rand_talk_release(_: &str, _vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    Pal4FunctionState::Completed
}

fn talk_wait(_: &str, _vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    Pal4FunctionState::Completed
}

fn player_do_action(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, player: i32, action_str: i32, flag: i32, _sync: i32);

    let action = get_str(vm, action_str as usize).unwrap();
    vm.vm_context.player_do_action(player, &action, flag);
...
fn player_current_do_action(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, action_str: i32, flag: i32, _sync: i32);

    let action = get_str(vm, action_str as usize).unwrap();
    vm.vm_context.player_do_action(-1, &action, flag);
...
fn player_current_set_pos(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, x:f32, y:f32, z:f32);
    vm.vm_context.set_player_pos(-1, &Vec3::new(x, y, z));
    Pal4FunctionState::Completed
}

fn player_set_rot(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
...
fn player_set_ang(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, player_id: i32, ang: f32);
    vm.vm_context.set_player_ang(player_id, ang);
    Pal4FunctionState::Completed
}

fn player_current_set_ang(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, ang: f32);

    vm.vm_context.set_player_ang(-1, ang);

    Pal4FunctionState::Completed
}

fn player_face_to_player(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, player1_id: i32, player2_id: i32, _sync: i32);
    let target = vm.vm_context.get_player_pos(player2_id);
    vm.vm_context.face_player_to_pos(player1_id, &target);
    Pal4FunctionState::Completed
}

fn player_set_dir(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, player_id: i32, direction: f32, sync: i32);

    vm.vm_context.player_rotate_to(player_id, direction);

    if sync == 1 && vm.vm_context.player_rotating(player_id) {
        Pal4FunctionState::Yield(Box::new(move |vm, _| {
            if vm.vm_context.player_rotating(player_id) {
                ContinuationState::Loop
            } else {
                ContinuationState::Completed
            }
...
fn player_face_to_npc(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, player_id: i32, npc_str: i32, _sync: i32);
    let name = get_str(vm, npc_str as usize).unwrap_or_default();
    if let Some(target) = vm.vm_context.npc_pos(&name) {
        vm.vm_context.face_player_to_pos(player_id, &target);
    }
    Pal4FunctionState::Completed
}
...
fn wait_player_move(
    ...
    if sync == 1 && vm.vm_context.player_moving(player_id) {
        Pal4FunctionState::Yield(Box::new(move |vm, _| {
            if vm.vm_context.player_moving(player_id) {
                ContinuationState::Loop
            } else {
                ContinuationState::Completed
            }
...
fn wait_npc_move(
    ...
    if sync == 1 && vm.vm_context.npc_moving(&npc_name) {
        Pal4FunctionState::Yield(Box::new(move |vm, _| {
            if vm.vm_context.npc_moving(&npc_name) {
                ContinuationState::Loop
            } else {
                ContinuationState::Completed
            }
...
fn player_walk_to(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
...
fn player_run_to(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, player_id:i32, x:f32, y:f32, z:f32, sync:i32);
...
fn player_current_walk_to(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, x: f32, y: f32, z: f32, sync: i32);
    vm.vm_context.player_to(-1, &Vec3::new(x, y, z), false);
    wait_player_move(vm, -1, sync)
}

fn player_back_to(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, player_id:i32, x:f32, y:f32, z:f32, sync:i32);
...
fn player_blend_out(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm,_player_id:i32,_blend_out_time:f32,_blend_out :i32);
    Pal4FunctionState::Completed
}

fn player_blend_in(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm,_player_id:i32,_blend_in_time:f32,_blend_in :i32);
    Pal4FunctionState::Completed
}

fn player_face_to_current_player(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, player_id: i32, _sync: i32);
    let target = vm.vm_context.get_player_pos(-1);
    vm.vm_context.face_player_to_pos(player_id, &target);
    Pal4FunctionState::Completed
}

fn current_player_face_to_npc(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, npc_str: i32, _sync: i32);
    let name = get_str(vm, npc_str as usize).unwrap_or_default();
    if let Some(target) = vm.vm_context.npc_pos(&name) {
        vm.vm_context.face_player_to_pos(-1, &target);
    }
    Pal4FunctionState::Completed
}

fn player_do_action_repeat(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, player: i32, action_str: i32);
    let action = get_str(vm, action_str as usize).unwrap();
    vm.vm_context.player_do_action(player, &action, 0);

    Pal4FunctionState::Completed
}

fn player_end_action_repeat(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, player_id: i32);
    vm.vm_context.player_unhold_act(player_id);
    Pal4FunctionState::Completed
}

fn player_end_move(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, player_id:i32);
...
fn current_player_end_move(_: &str, _vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    Pal4FunctionState::Yield(Box::new(move |vm, _delta_sec| {
        if vm.vm_context.player_moving(-1) {
            ContinuationState::Loop
        } else {
            ContinuationState::Completed
        }
...
fn npc_walk_to(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, npc_file_str: i32, x: f32, y: f32, z: f32, sync: i32);

    let npc_name = get_str(vm, npc_file_str as usize).unwrap();
    vm.vm_context.npc_to(&npc_name, &Vec3::new(x, y, z), false);
    wait_npc_move(vm, npc_name, sync)
}

fn npc_run_to(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, npc_file_str: i32, x: f32, y: f32, z: f32, sync: i32);

    let npc_name = get_str(vm, npc_file_str as usize).unwrap();
    vm.vm_context.npc_to(&npc_name, &Vec3::new(x, y, z), true);
    wait_npc_move(vm, npc_name, sync)
}

fn npc_back_to(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, npc_file_str: i32, x: f32, y: f32, z: f32, sync: i32);
...
fn npc_set_pos(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, npc_str: i32, x: f32, y: f32, z: f32);
    let name = get_str(vm, npc_str as usize).unwrap_or_default();
    vm.vm_context.npc_set_pos(&name, &Vec3::new(x, y, z));
    Pal4FunctionState::Completed
}

fn npc_set_rot(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, npc_str: i32, rot_str: i32);
    // PAL4 passes a string token resolved via lookup tables we don't
    // model yet. Forward numeric literals; otherwise leave the
    // rotation untouched.
    if let Some(s) = get_str(vm, rot_str as usize) {
        if let Ok(ang) = s.trim().parse::<f32>() {
            let name = get_str(vm, npc_str as usize).unwrap_or_default();
            vm.vm_context.npc_set_ang(&name, ang);
        }
    }
    Pal4FunctionState::Completed
}

fn npc_set_dir(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, npc_str: i32, direction: f32, sync: i32);
    let name = get_str(vm, npc_str as usize).unwrap_or_default();
    vm.vm_context.npc_rotate_to(&name, direction);

    if sync == 1 && vm.vm_context.npc_rotating(&name) {
        Pal4FunctionState::Yield(Box::new(move |vm, _| {
            if vm.vm_context.npc_rotating(&name) {
                ContinuationState::Loop
            } else {
                ContinuationState::Completed
            }
...
fn npc_face_to_npc(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, npc1_str: i32, npc2_str: i32, _sync: i32);
    let name1 = get_str(vm, npc1_str as usize).unwrap_or_default();
    let name2 = get_str(vm, npc2_str as usize).unwrap_or_default();
    if let Some(target) = vm.vm_context.npc_pos(&name2) {
        vm.vm_context.face_npc_to_pos(&name1, &target);
    }
    Pal4FunctionState::Completed
}

fn npc_face_to_player(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, npc_str: i32, player_id: i32, _sync: i32);
    let name = get_str(vm, npc_str as usize).unwrap_or_default();
    let target = vm.vm_context.get_player_pos(player_id);
    vm.vm_context.face_npc_to_pos(&name, &target);
    Pal4FunctionState::Completed
}
...
fn npc_end_move(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, npc_file_str:i32);
...
fn npc_set_ang(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, npc_str: i32, ang: f32);
    let name = get_str(vm, npc_str as usize).unwrap_or_default();
    vm.vm_context.npc_set_ang(&name, ang);
    Pal4FunctionState::Completed
}

fn camera_prepare(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, file_str: i32);
    let file_name = get_str(vm, file_str as usize).unwrap();

    match vm.vm_context.prepare_camera(&file_name) {
        Err(e) => {
...
fn camera_run_single(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, camera_data: i32, sync: i32);
    let name = get_str(vm, camera_data as usize).unwrap();

    let running = vm.vm_context.start_camera_run(&name);

    vm.set_ret_value(1);

    if sync == 1 && running {
        Pal4FunctionState::Yield(Box::new(move |vm, _| {
            if vm.vm_context.camera_running() && !vm.vm_context().fast_forward() {
                ContinuationState::Loop
            } else {
                ContinuationState::Completed
            }
...
fn camera_run_circle(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
...
fn camera_wait(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    if vm.vm_context.camera_running() {
        Pal4FunctionState::Yield(Box::new(move |vm, _| {
            if vm.vm_context.camera_running() && !vm.vm_context().fast_forward() {
                ContinuationState::Loop
            } else {
...
fn flash_out_black(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    ...
    if sync == 1 {
        Pal4FunctionState::Yield(Box::new(move |vm, _| {
            if vm.vm_context().fast_forward() {
                vm.vm_context.set_actdrop(InterpValue::new(1., 1., 0.));
                return ContinuationState::Completed;
            }
            if vm.vm_context.get_actdrop().current() == 1. {
                ContinuationState::Completed
            } else {
                ContinuationState::Loop
            }
...
fn flash_in_black(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    ...
    if sync == 1 {
        Pal4FunctionState::Yield(Box::new(move |vm, _| {
            if vm.vm_context().fast_forward() {
                vm.vm_context.set_actdrop(InterpValue::new(0., 0., 0.));
                return ContinuationState::Completed;
            }
            if vm.vm_context.get_actdrop().current() == 0. {
                ContinuationState::Completed
            } else {
                ContinuationState::Loop
            }
...
fn flash_out_white(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, _flash_time:f32,_flash_out_white1 :i32,_flash_out_white2 :i32);
    Pal4FunctionState::Completed
}

fn flash_in_white(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, _flash_time:f32,_flash_in_white :i32);
    Pal4FunctionState::Completed
}

fn flash_out_red(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, _flash_time:f32,_flash_out_red1 :i32,_flash_out_red2 :i32);
    Pal4FunctionState::Completed
}

fn flash_in_red(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
...
fn play_movie(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, movie_file_str:i32);

    let movie_name = get_str(vm, movie_file_str as usize).unwrap();
    let source_size = match vm.vm_context.start_play_movie(&movie_name) {
        Some(size) => size,
        None => {
    ...
    Pal4FunctionState::Yield(Box::new(move |vm, _| {
        ...
        let video_player = vm.vm_context.video_player();
...
fn start_trade_system(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm,_trade_file_str1:i32,_trade_file_str2:i32);
    Pal4FunctionState::Completed
}

fn start_puzzle_game(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm,_puzzle_id:i32);
    Pal4FunctionState::Completed
}

fn start_jigsaw_game(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
...
fn mst_blend_out(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm,_mst_file_str:i32,_blend_out_time:f32,_blend_out :i32);
    Pal4FunctionState::Completed
}

fn mst_blend_in(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm,_mst_file_str:i32,_blend_in_time:f32,_blend_in :i32);
    Pal4FunctionState::Completed
}

fn show_common_dialog(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm,_dialog_file_str:i32,_x:f32,_y:f32);
    Pal4FunctionState::Completed
}

fn show_select_dialog(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm,_dialog_file_str:i32);
    Pal4FunctionState::Completed
}
fn gob_movment(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
...
fn show_tutorial(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, _key_id: i32);
    Pal4FunctionState::Completed
}

fn show_world_map(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    ...
    vm.vm_context.session().open_world_map();
...
fn gob_scale(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
...
fn player_current_face_to_gob(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm,_gob_file_str:i32,_face_to_gob :i32);
    Pal4FunctionState::Completed
}

fn player_current_movement(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm,_x:f32,_y:f32,_z:f32,_rot:f32,_movment :i32);
    Pal4FunctionState::Completed
}

fn show_poetry(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm,_poetry_id:i32,_show_poetry :i32);
    Pal4FunctionState::Completed
}

fn npc_fly_to(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm,_npc_file_str:i32,_x:f32,_y:f32,_z:f32,_fly_to :i32);
    Pal4FunctionState::Completed
}

fn goto_logo_wait(_: &str, _vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    Pal4FunctionState::Completed
}
...
fn play_movie_final(_: &str, vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    as_params!(vm, _movie_file_str:i32);
    Pal4FunctionState::Completed
}

fn unknown(_: &str, _vm: &mut ScriptVm<Pal4VmContext>) -> Pal4FunctionState {
    Pal4FunctionState::Completed
}
