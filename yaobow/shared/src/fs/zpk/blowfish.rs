// Blowfish zpk-variant implementation
// Derived work from https://github.com/RustCrypto/block-ciphers
...
impl<T: ByteOrder> Blowfish<T> {
    ...
    fn expand_key(&mut self, key: &[u8]) {
        let mut key_pos = 0;
        let round = 18.max((key.len() + 3) / 4);
        for i in 0..round {
            self.p[i % 18] ^= next_u32_wrap(key, &mut key_pos);
        }
