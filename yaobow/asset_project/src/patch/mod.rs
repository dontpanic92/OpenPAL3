//! `.ybpatch`: a `.ypk`-based delta-pack format.
//!
//! A `.ybpatch` file is a regular `.ypk` archive (see
//! `ypk`) that always carries three kinds of entries:
//!
...
//!
//! Reusing the `.ypk` container means `.ybpatch` gets zstd compression,
//! path-hash lookup, and a battle-tested reader/writer for free instead
//! of a bespoke wire format.
//!
//! Publishing a `.ybpatch` (see [`YbpatchWriter::finish`] and
...
pub use reader::YbpatchReader;
pub use writer::{YbpatchWriter, publish};
...
/// read.
pub const YBPATCH_FORMAT_VERSION: u32 = 1;

pub(crate) const YBPATCH_MANIFEST_ENTRY: &str = "manifest.json";
pub(crate) const YBPATCH_MANIFEST_HASH_ENTRY: &str = "manifest.hash";
pub(crate) const YBPATCH_PAYLOAD_PREFIX: &str = "payload/";
...
/// Describes the set of [`AssetChange`]s carried by one `.ybpatch`
