...
fn main() {
    ...
    // Collapsed 16x16 coverage map (max density per cell over all leaves), as
    // grass.rs builds it. Shows how much of the block is flagged as grass.
    println!("\n--- collapsed 16x16 coverage (max density per cell) ---");
    let mut cov = [[0u8; 16]; 16]; // [row][col]
    for l in &ctr.leaves {
        if l.density.is_empty() {
            continue;
        }
        for row in l.g[1]..=l.g[3] {
            for col in l.g[0]..=l.g[2] {
                if row < 0 || col < 0 || row >= 16 || col >= 16 {
                    continue;
                }
                if let Some(d) = l.density_at(col, row) {
                    let c = &mut cov[row as usize][col as usize];
                    *c = (*c).max(d);
                }
            }
        }
    ...
    for row in &cov {
        ...
        for &d in row {
            if d == 0 {
                ...
                covered += 1;
                line.push((b'0' + d.min(9)) as char);
            }
        }
        println!("  {line}");
    }
    println!("covered cells: {covered}/256");
}
